...
class NetworkBenchmark:
    ...
    def inject_input(self, layer_idx: int = 0):
        ...
        if self.network and f"layer_{layer_idx}" in self.network.layers:
            ...
            # Generate random input currents (10-20 nA)
            input_currents = np.random.uniform(10, 20, layer.size)
    ...
    def run_simulation(self) -> Tuple[float, int]:
        ...
        for step in range(num_steps):
            ...
            # Inject input to first layer every 10ms
            if step % int(10.0 / self.config.dt) == 0:
                if "layer_0" in self.network.layers:
                    input_layer = self.network.layers["layer_0"]
                    input_currents = self.inject_input(0)
                    # Direct injection via neuron population
                    for i, current in enumerate(input_currents[:len(input_layer.neuron_population.neurons)]):
                        # Add current directly to membrane potential
                        input_layer.neuron_population.neurons[i].membrane_potential += current * 0.1
            ...
            for layer in self.network.layers.values():
                for neuron in layer.neuron_population.neurons:
                    if neuron.is_spiking:
                        step_spikes += 1
                        neuron.is_spiking = False  # Reset spike flag
...
class GPUBenchmark(NetworkBenchmark):
    ...
    def __init__(self, config: BenchmarkConfig, backend='torch'):
        super().__init__(config)
    ...
    def _move_to_gpu_torch(self, network):
    ...
    def _move_to_gpu_cupy(self, network):
...
class BenchmarkRunner:
    ...
    def run_single_benchmark(self, config: BenchmarkConfig) -> BenchmarkResult:
        ...
        try:
            ...
            def run_wrapper():
                return benchmark.run_simulation()
            
            # Run with memory profiling
            mem_usage = memory_usage(run_wrapper, interval=0.1, timeout=None, retval=True)
            memory_data, (wall_clock_time, total_spikes) = mem_usage
            
            # Calculate metrics
            memory_peak = max(memory_data)
            memory_average = np.mean(memory_data)
            ...
            if benchmark.step_times:
                step_time_mean = np.mean(benchmark.step_times)
                step_time_std = np.std(benchmark.step_times)
            else:
                step_time_mean = 0.0
                step_time_std = 0.0
    ...
    def run_benchmark_suite(self):
        ...
        for size in network_sizes:
            for platform in platforms:
                ...
                if size <= 1000:
                    num_layers = 3
                    sim_time = 1000.0  # 1 second
                    conn_prob = 0.1
                elif size <= 10000:
                    num_layers = 4
                    sim_time = 500.0  # 0.5 seconds
                    conn_prob = 0.05
                else:  # 50000
                    num_layers = 5
                    sim_time = 200.0  # 0.2 seconds
                    conn_prob = 0.02
