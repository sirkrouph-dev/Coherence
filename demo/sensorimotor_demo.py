...
from api.neuromorphic_api import NeuromorphicAPI, SensorimotorSystem
import os
import sys
...
class SensorimotorSystem:
    ...
    def setup_network(self):
        ...
        # Sensory layers (much smaller)
        self.api.add_sensory_layer("visual_input", 64, "retinal")
        self.api.add_sensory_layer("auditory_input", 32, "cochlear")
        self.api.add_sensory_layer("tactile_input", 32, "somatosensory")

        # Processing layers (much smaller)
        self.api.add_processing_layer("sensory_integration", 64, "adex")
        self.api.add_processing_layer("pattern_recognition", 32, "adex")
        self.api.add_processing_layer("decision_making", 16, "adex")

        # Motor layers (much smaller)
        self.api.add_motor_layer("motor_planning", 16)
        self.api.add_motor_layer("motor_output", 8)
    ...
    def train(
        self, training_data: List[Dict[str, Any]], epochs: int = 10
    ) -> Dict[str, Any]:
...
def demonstrate_sensorimotor_learning():
    ...
    training_results = system.train(training_data, epochs=5)  # Reduced from 50
...
def demonstrate_adaptive_behavior():
    ...
    for trial in range(5):
        ...
        print(
            f"Trial {trial + 1}: {len(results['layer_spike_times'].get('motor', [[]]))} motor spikes"
        )
