...
class AdaptiveExponentialIntegrateAndFire(NeuronModel):
    ...
    def step(self, dt: float, I_syn: float) -> bool:
        ...
        dv_dt = (-(self.membrane_potential - self.v_rest) + 
                 ...
                 self.adaptation_current + I_syn) / self.tau_m
        self.membrane_potential += dv_dt * dt
        
        # Update adaptation current
        dw_dt = (self.a * (self.membrane_potential - self.v_rest) - self.adaptation_current) / self.tau_w
    ...
    def _spike(self):
        ...
        self.refractory_time = self.refractive_period
...
class HodgkinHuxleyNeuron(NeuronModel):
    ...
    def __init__(self, neuron_id: int,
                 ...
                 E_L: float = -54.4):    # Leak reversal potential (mV)
        """
