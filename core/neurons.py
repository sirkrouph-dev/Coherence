...
class AdaptiveExponentialIntegrateAndFire(NeuronModel):
    ...
    def step(self, dt: float, I_syn: float) -> bool:
        ...
        # Update membrane potential with proper current scaling
        # Scale input current appropriately (typical input resistance ~100 MOhm)  
...
class LeakyIntegrateAndFire(NeuronModel):
    ...
    def step(self, dt: float, I_syn: float) -> bool:
        ...
        # Efficient bounds checking - only clamp if needed (avoids np.clip overhead)
        if self.membrane_potential < -120.0:
            self.membrane_potential = -120.0
        elif self.membrane_potential > 50.0:
            self.membrane_potential = 50.0
...
class NeuronPopulation:
    ...
    def __init__(self, size: int, neuron_type: str = "adex", **kwargs):
        ...
        # Cache for spike states to avoid repeated list comprehensions
        self.spike_states = [False] * size
    ...
    def step(self, dt: float, I_syn: List[float]) -> List[bool]:
        """
        Advance all neurons by one time step with vectorized processing.
        ...
        if len(I_syn) != self.size:
            raise ValueError(
                f"Expected {self.size} synaptic currents, got {len(I_syn)}"
            )

        # HIGHLY OPTIMIZED: Vectorized processing with minimal function calls
        spikes = []

        # Process all neurons with optimized loop
        for i, (neuron, current) in enumerate(zip(self.neurons, I_syn)):
            spiked = neuron.step(dt, current)
            spikes.append(spiked)
            neuron.is_spiking = spiked  # Cache spike state for fast access
            self.spike_states[i] = spiked  # Cache in array for ultra-fast access

        return spikes

    def get_spike_states(self) -> List[bool]:
        """
        Get current spike states efficiently - VECTORIZED VERSION.

        Returns:
            List of current spike states for all neurons
        """
        return self.spike_states  # Return cached array directly

    def reset_spike_flags(self):
        """
        Reset all spike flags efficiently - VECTORIZED VERSION.
        """
