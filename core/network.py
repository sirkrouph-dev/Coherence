...
class NetworkConnection:
    ...
    def __init__(self, pre_layer: str, post_layer: str, synapse_type: str = "stdp",
    ...
    def initialize(self, pre_size: int, post_size: int):
        ...
        self.synapse_population = SynapsePopulation(
            pre_size, post_size, self.synapse_type, 
            self.connection_probability, **self.kwargs
        )
    ...
    def update_weights(self, pre_spikes: List[bool], post_spikes: List[bool], current_time: float):
        """Update synaptic weights."""
        if self.synapse_population is not None:
            self.synapse_population.update_weights(pre_spikes, post_spikes, current_time)
...
class NeuromorphicNetwork:
    ...
    def connect_layers(self, pre_layer: str, post_layer: str, 
                      synapse_type: str = "stdp", connection_probability: float = 0.1, **kwargs):
        """
        Connect two layers.
    ...
    def step(self, dt: float):
        ...
        self.simulation_history.append({
            ...
            'layer_potentials': {name: layer.get_membrane_potentials() 
    ...
    def run_simulation(self, duration: float, dt: float = 0.1) -> Dict[str, Any]:
        ...
        return {
            ...
            'layer_spike_times': {name: layer.get_spike_times() 
                                for name, layer in self.layers.items()},
            'weight_matrices': {conn_name: conn.get_weight_matrix() 
...
class EventDrivenSimulator:
    ...
    def add_external_input(self, layer_name: str, neuron_id: int, input_time: float, 
                          input_strength: float):
        """Add external input event."""
        heapq.heappush(self.event_queue, (input_time, "input", neuron_id, layer_name, input_strength))
    ...
    def run_simulation(self, duration: float) -> Dict[str, Any]:
        ...
        while self.event_queue and self.current_time < duration:
            ...
            if event_type == "spike":
                ...
                self._process_external_input(neuron_id, layer_name, input_strength, event_time)
        ...
        for (pre_name, post_name), connection in self.network.connections.items():
            if pre_name == layer_name:
                ...
                for post_neuron_id in range(post_layer.size):
                    if (neuron_id, post_neuron_id) in connection.synapse_population.synapses:
    ...
    def _process_external_input(self, neuron_id: int, layer_name: str, 
                               input_strength: float, input_time: float):
...
class NetworkBuilder:
    ...
    def connect_layers(self, pre_layer: str, post_layer: str, 
                      connection_type: str = "random", **kwargs):
        """Connect layers with specified pattern."""
        if connection_type == "random":
            self.network.connect_layers(pre_layer, post_layer, **kwargs)
        elif connection_type == "feedforward":
            self.network.connect_layers(pre_layer, post_layer, 
                                      connection_probability=0.3, **kwargs)
        elif connection_type == "lateral":
            # Lateral connections within the same layer
            self.network.connect_layers(pre_layer, post_layer, 
                                      connection_probability=0.1, **kwargs)
        elif connection_type == "feedback":
            self.network.connect_layers(pre_layer, post_layer, 
                                      connection_probability=0.2, **kwargs)
        return self
