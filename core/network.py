...
class NeuromorphicNetwork:
    ...
    def add_layer(self, name: str, size: int, neuron_type: str = "adex", **kwargs):
        ...
        if size > self.MAX_NEURONS:
            raise ValueError(
                f"Layer size {size} exceeds maximum allowed neurons {self.MAX_NEURONS}"
            )
        ...
        if self.total_neurons + size > self.MAX_NEURONS:
            raise ValueError(
                f"Adding layer would exceed total neuron limit of {self.MAX_NEURONS}"
            )
    ...
    def connect_layers(
        ...
        **kwargs,
        ...
        if not 0.0 <= connection_probability <= 1.0:
            raise ValueError(
                f"Connection probability must be between 0 and 1, got {connection_probability}"
            )
        ...
        if synapse_type not in valid_synapse_types:
            raise ValueError(
                f"Invalid synapse type '{synapse_type}'. Must be one of {valid_synapse_types}"
            )
        ...
        if self.total_synapses + expected_synapses > self.MAX_SYNAPSES:
            raise ValueError(
                f"Adding connection would exceed total synapse limit of {self.MAX_SYNAPSES}"
            )
        ...
        # Mark for vectorized re-initialization
        self._initialized_vectorized = False

    def _initialize_vectorized_structures(self):
        """Initialize optimized vectorized data structures for fast simulation."""
        if self._initialized_vectorized:
            return
            
        try:
            from scipy.sparse import csr_matrix
        except ImportError:
            # Fallback: create a simple sparse-like structure
            csr_matrix = None
        
        # Pre-allocate current and spike arrays for each layer
        for layer_name, layer in self.layers.items():
            self.layer_currents[layer_name] = np.zeros(layer.size, dtype=np.float32)
            self.layer_spikes[layer_name] = np.zeros(layer.size, dtype=bool)
        
        # Convert connections to sparse matrices
        for (pre_name, post_name), connection in self.connections.items():
            if connection.synapse_population is not None:
                pre_size = self.layers[pre_name].size
                post_size = self.layers[post_name].size
                
                if csr_matrix is not None:
                    # Use scipy sparse matrix for optimal performance
                    weight_matrix = connection.get_weight_matrix()
                    if weight_matrix is not None:
                        self.connections_sparse[(pre_name, post_name)] = csr_matrix(weight_matrix)
                    else:
                        # Create sparse matrix from synapse population
                        row_indices = []
                        col_indices = []
                        weights = []
                        ...
                        self.connections_sparse[(pre_name, post_name)] = csr_matrix(
                            (weights, (row_indices, col_indices)), 
                            shape=(post_size, pre_size),
                            dtype=np.float32
                        )
                else:
                    # Fallback: use dense numpy matrix
                    weight_matrix = connection.get_weight_matrix()
                    if weight_matrix is not None:
                        self.connections_sparse[(pre_name, post_name)] = weight_matrix.astype(np.float32)
                    else:
                        # Create dense matrix from connections
                        dense_matrix = np.zeros((post_size, pre_size), dtype=np.float32)
                        for (pre_idx, post_idx), synapse in connection.synapse_population.synapses.items():
                            dense_matrix[post_idx, pre_idx] = synapse.weight
                        self.connections_sparse[(pre_name, post_name)] = dense_matrix
        
        self._initialized_vectorized = True

    def _update_sparse_matrix(self, pre_name: str, post_name: str, connection: NetworkConnection):
        """Update sparse matrix with new weights after plasticity updates."""
    ...
    def step(self, dt: float):
        """Advance network by one time step with adaptive optimization."""
        # Use fast path for all networks - vectorization overhead is too high
        return self._step_fast_path(dt)
    
    def _step_fast_path(self, dt: float):
        """Fast path for small networks - optimized Python loops."""
        # Initialize layer currents
        layer_currents = {}
        for layer_name, layer in self.layers.items():
            layer_currents[layer_name] = [0.0] * layer.size

        # Compute synaptic currents (optimized)
        for (pre_name, post_name), connection in self.connections.items():
            if connection.synapse_population is not None:
                pre_layer = self.layers[pre_name]
                
                # Ultra-fast spike detection using optimized method
                pre_spikes = pre_layer.neuron_population.get_spike_states()
                
                # Fast synaptic current computation
                currents = connection.get_synaptic_currents(pre_spikes, self.current_time)
                
                # Efficient current accumulation
                post_currents = layer_currents[post_name]
                for i, current in enumerate(currents[:len(post_currents)]):
                    post_currents[i] += current
        ...
        for (pre_name, post_name), connection in self.connections.items():
            if pre_name in layer_spikes and post_name in layer_spikes:
                connection.update_weights(layer_spikes[pre_name], layer_spikes[post_name], self.current_time)
        ...
        # Minimal history recording
        if int(self.current_time / dt) % 100 == 0:
            self.simulation_history.append({
                "time": self.current_time,
                "layer_spikes": layer_spikes.copy(),
            })

    def _step_vectorized(self, dt: float):
        """Vectorized path for large networks - NumPy operations."""
        # Initialize vectorized structures if needed
        self._initialize_vectorized_structures()
        
        # Reset currents
        for currents in self.layer_currents.values():
            currents.fill(0.0)

        # Precompute spike vectors
        for layer_name, layer in self.layers.items():
            if hasattr(layer.neuron_population, 'get_spike_states'):
                spike_states = layer.neuron_population.get_spike_states()
                self.layer_spikes[layer_name][:] = spike_states
            else:
                for i, neuron in enumerate(layer.neuron_population.neurons):
                    self.layer_spikes[layer_name][i] = getattr(neuron, 'is_spiking', False)

        # Vectorized synaptic currents
        for (pre_name, post_name), W_sparse in self.connections_sparse.items():
            pre_spikes = self.layer_spikes[pre_name]
            
            if hasattr(W_sparse, 'dot'):
                synaptic_currents = W_sparse.dot(pre_spikes.astype(np.float32))
            else:
                synaptic_currents = W_sparse @ pre_spikes.astype(np.float32)
            
            self.layer_currents[post_name] += synaptic_currents

        # Update neurons
        layer_spikes = {}
        for layer_name, layer in self.layers.items():
            currents = self.layer_currents[layer_name].tolist()
        ...
        for (pre_name, post_name), connection in self.connections.items():
            if pre_name in layer_spikes and post_name in layer_spikes:
                connection.update_weights(layer_spikes[pre_name], layer_spikes[post_name], self.current_time)
                if (pre_name, post_name) in self.connections_sparse:
                    self._update_sparse_matrix(pre_name, post_name, connection)
    ...
    def run_simulation(self, duration: float, dt: float = 0.1) -> Dict[str, Any]:
        ...
        if dt > duration:
            raise ValueError(
                f"Time step {dt} cannot be larger than duration {duration}"
            )

        num_steps = int(duration / dt)
        if num_steps > self.MAX_SIMULATION_STEPS:
            raise ValueError(
                f"Simulation would require {num_steps} steps, exceeding limit of {self.MAX_SIMULATION_STEPS}"
            )
    ...
    def reset(self):
        ...
        # Reset vectorized structures
        for currents in self.layer_currents.values():
            currents.fill(0.0)
        for spikes in self.layer_spikes.values():
            spikes.fill(False)
        
        # Mark for re-initialization if structure changed
        self._initialized_vectorized = False
                ...
                for (pre_name, post_name), connection in self.connections.items():
                    ...
                    for i, neuron in enumerate(pre_layer.neuron_population.neurons):
                        if hasattr(neuron, "is_spiking") and neuron.is_spiking:
                            pre_spikes[i] = True
                    currents = connection.get_synaptic_currents(
                        pre_spikes, self.current_time
                    )
                ...
                if noise_std is not None and noise_std > 0.0:
                    for lname, layer in self.layers.items():
                        ...
                        if lc:
                            ...
                            lc_arr += np.random.normal(
                                0.0, float(noise_std), size=layer.size
                            )
                            # write back
                            for i in range(layer.size):
                                lc[i] = float(lc_arr[i])
                ...
                for (pre_name, post_name), connection in self.connections.items():
                    if pre_name in layer_spikes and post_name in layer_spikes:
                        connection.update_weights(
                            layer_spikes[pre_name],
                            layer_spikes[post_name],
                            self.current_time,
                        )
...
class EventDrivenSimulator:
    ...
    def _process_spike(self, neuron_id: int, layer_name: str, spike_time: float):
        ...
        if self.network is None or layer_name not in self.network.layers:
            return
        ...
        for (pre_name, post_name), connection in self.network.connections.items():
            if pre_name == layer_name:
                ...
                # Create spike events for postsynaptic neurons (if synapse_population exists)
                if connection.synapse_population is not None:
