...
class STDP_Synapse(SynapseModel):
    ...
    def post_spike(self, spike_time: float):
        ...
        if spike_time - self.last_pre_spike < self.tau_stdp:
            ...
            # Log synapse weight change (only for first few synapses to avoid spam)
            if self.synapse_id < 10:  # Only log first 10 synapses
                neuromorphic_logger.log_synapse_activity(
            ...
            for post_id in range(post_population_size):
                if np.random.random() < connection_probability:
                    synapse = SynapseFactory.create_synapse(
                        synapse_id, pre_id, post_id, synapse_type, **kwargs
                    )
