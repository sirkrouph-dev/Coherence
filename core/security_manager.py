...
class SecurityManager:
    ...
    def validate_array_input(
        ...
        if array.size > SecurityManager.MAX_ARRAY_SIZE:
            raise ValueError(
                f"Array too large: {array.size} > {SecurityManager.MAX_ARRAY_SIZE}"
            )

        # Check shape if specified
        if shape is not None and array.shape != shape:
            raise ValueError(
                f"Invalid array shape: expected {shape}, got {array.shape}"
            )
    ...
    @staticmethod
    def safe_divide(
        numerator: np.ndarray, denominator: np.ndarray, epsilon: float = 1e-10
    ) -> np.ndarray:
        """
        ...
        safe_denominator = np.where(
            np.abs(denominator) < epsilon, np.sign(denominator) * epsilon, denominator
        )
...
class RateLimiter:
    ...
    def check_rate_limit(self) -> bool:
        ...
        self.call_times = [
            t for t in self.call_times if current_time - t < self.time_window
        ]
