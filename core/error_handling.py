...
def validate_shape(
    ...
    if min_dims is not None and ndims < min_dims:
        raise ValidationError(
            f"{name} has {ndims} dimensions, minimum is {min_dims}"
        )
    
    if max_dims is not None and ndims > max_dims:
        raise ValidationError(
            f"{name} has {ndims} dimensions, maximum is {max_dims}"
        )
...
class NumericalStabilizer:
    ...
    def safe_exp(x: np.ndarray) -> np.ndarray:
        ...
        return np.exp(np.clip(x, -NumericalStabilizer.MAX_EXP, NumericalStabilizer.MAX_EXP))
...
def setup_error_handling(log_file: Optional[str] = None):
    ...
    def exception_hook(exc_type, exc_value, exc_traceback):
        ...
        global_error_handler.handle_error(
            exc_value,
            {"type": exc_type.__name__, "traceback": traceback.format_tb(exc_traceback)},
            severity="CRITICAL"
        )
